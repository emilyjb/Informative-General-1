...
repeat{
	...
	ub <- rnorm(length(unique(areafacsamp)), mean = 0, sd = sqrt(sigma2u))
	names(ub) <- sort(unique(areafacsamp))
	...
	repeat{
		...
		r <- r + 1
		ugenrb <- sapply(sort(unique(areafacsamp)), genudlogist ,parb[3], parb[c(1,2)], xs, ys, areafacsamp, 100)
